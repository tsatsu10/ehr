...
| ------- | ---- | ------- |
| 0.1.15 | 2026-07-15 | **Self-audit fixes on the 0.1.14 work (4 issues).** (1) *[High]* closed an amendment-gate bypass — an already-released order (or one just released in-session) showed a plain "Release to doctor" that re-released with no reason; re-release is now offered only for a first release or an in-progress correction, and `already_released` is set after release. (2) *[Med]* "Amend result" is hidden from enter-only users (needs release privilege; was 403-ing on click). (3) *[Med]* QC-range overrides now derive `reference_range` from the tuned warn bounds server-side, so the result form's Range box can't disagree with the flagging bounds. (4) *[Low]* mid-correction the button reads "Edit results" and Release stays available. Verified: PHP + Vitest green, `reference_range` derivation smoke-tested live (client sent `7–18`, stored `8–16`). |
| 0.1.14 | 2026-07-15 | **Lab-standard hardening (7 recommendations).** (1) **Age/sex QC bands (D-LAB-AGE)** — `LabResultValidationService` resolves paediatric (infant/child) and adult male/female reference ranges by patient age/sex (DOB+sex threaded from the order); the client rules carry the resolved band. (2) **Critical-value notification (D-LAB-CRIT)** — releasing a critical result opens a read-back modal (who/how/read-back confirmed) logged to `new_lab_critical_notification`; release still never blocks (pending row if skipped). (3) **Day summary header (D-LAB-TAT)** — worklist shows orders/released/**median turnaround**/**rejection rate** for the date. (4) **Audit trail** — verified lab audits *do* persist (`event=new_clinic`, action in `user`); added `patient_id` to release/reject/save/collect/send-out events so they are patient-filterable. (5) **Admin-editable QC ranges (D-LAB-QC)** — `new_lab_qc_rule` override store + lab-lead editor in the setup panel; overrides win over built-in defaults. (6) **Released-result amendment (D-LAB-AMEND)** — reason-gated correction: snapshots the original to `new_lab_result_amendment`, reopens for edit, marks reports `corrected` on re-release. (7) **Validator off the DOM (D-LAB-VALIDATE)** — `labResultValidation.ts` is now a pure state-based module (no `querySelector`/feedback DOM writes); the drawer renders feedback from React state. Tests: +PHP (QC rules, age bands, amend guards) +Vitest (validator, summary bar, QC editor, critical/amend gates). |
| 0.1.13 | 2026-07-15 | **Specimen rejection log (D-LAB-REJECT) + turnaround time (D-LAB-TAT)** — closes the two remaining SLIPTA operational indicators. Rejection: `new_lab_specimen_rejection` log table + `rejected_at`/`rejection_reason` on `new_lab_order_meta`; `rejectSpecimen` (8 standard reasons) clears collected state for re-collection; `lab_ops.specimen_reject` action (enter ACL); worklist reject action + reason modal. TAT: worklist `tat_label` (order→release when done; elapsed "In lab"/"Waiting" in progress). Also `window.prompt` → inline input. Verified live end-to-end (seeded order → reject in browser → DB log + meta clear confirmed). |
| 0.1.12 | 2026-07-15 | **Critical-value flag (D-LAB-CRIT)** + standards benchmark. Audited against ISO 15189 / WHO-SLIPTA: the controlled, audited, dual-ACL result-release workflow is up to standard; the missing pieces were SLIPTA's operational indicators. Implemented critical/panic-value detection (`crit_min`/`crit_max` + qualitative `critical_values` in `LabResultValidationService`; standard adult panic thresholds; loud red drawer callout live + on release; release audit + response record which tests were critical; never blocks). Replaced 8 `window.alert()` with `showDeskToast`. **Still open:** specimen-rejection log + turnaround-time monitoring (the other two SLIPTA indicators); `window.prompt` → input modal. |
| 0.1.11 | 2026-07-15 | **Order form coherence fixes** — (1) editing an order now bills tests **added** since the last save (was a revenue leak: added tests went unbilled); existing tests still never double-charged; (2) Clinical history pre-fills from the visit's chief complaint on a new order; (3) removed the orphaned order-level "Specimen volume" field (specimen is per-test now); (4) "Order diagnosis" → "Diagnosis / reason for test" with a realistic placeholder. **Deferred:** test panels/bundles; send-out lab pricing. |
| 0.1.10 | 2026-07-14 | **Per-test specimen** — native proc-order form (M13) now lets staff pick a specimen per test line, not one per order; stored in module side table `new_lab_order_line_specimen` (core `procedure_order_code` untouched); order-level `procedure_order.specimen_type` mirrors the first line for stock parity; requisition print shows each test's specimen. Specimen volume stays order-level. |
