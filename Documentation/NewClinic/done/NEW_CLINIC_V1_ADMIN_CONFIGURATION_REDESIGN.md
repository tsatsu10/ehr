...
| **Status** | Draft for review — **Module M6 + M15** integrated in PRD v1.20.32; PAGE_DESIGNS §7.27–§7.28; USER_WORKFLOWS §14.8; PRD §17.4.8 / §19.4 / §21.1v; **D-ADMIN-1–5 closed**; nav model redesigned per `NEW_CLINIC_ADMIN_HUB_APPLE_REDESIGN_PLAN.md` (ADM-1..7 shipped) |
...
|---------|------|---------|
| 0.2.0 | 2026-07-19 | **Apple-theme redesign (ADM-1..7, `NEW_CLINIC_ADMIN_HUB_APPLE_REDESIGN_PLAN.md`):** the wrapped 10-tab strip is replaced by a sticky grouped left-rail nav (Get started / Clinic / People & money / Operations); the old "Queue & roles" mega-tab split into **Queue & desks** + **Features**; Setup checklist promoted to its own top-level destination (was a scroll-anchor inside System — legacy `?tab=system#nc-admin-setup-checklist` links still resolve there); global settings search (field + destination results, jump-and-highlight); unsaved-changes guard now covers every navigation (section switch, search jump, tab close), not just the scope picker; one-accent visual pass (green reserved for genuine done/success only, flat `#0071e3` for every action/nav-active state) — no more `oe-cta` green buttons or a hardcoded-green icon regardless of tone. **New: per-clinic override transparency** — any setting with an explicit facility-level row shows an "Overridden for this clinic" indicator + "Use global value" reset. Building that surfaced a real pre-existing bug: the settings form has always saved its **entire** state on every Save, and the backend wrote every key unconditionally — so the first save at any facility silently gave every setting its own facility row forever, forever masking the actual override signal. Fixed by writing only when a value genuinely changes; resolution behavior is unaffected, only which rows physically exist. |
| 0.1.9 | 2026-07-18 | **Handover print + cron copy + E2E:** (1) the one-time credentials panel gains **Print handover sheet** — cut-apart slips per person (sign-in, temp password, first-login steps, destroy-after note) via a hidden `srcdoc` iframe (no `document.write`, no popup blocker); (2) cron hint now says a manual Run-reconcile does not count; (3) new Playwright golden path `setup-checklist.spec.js` (deep link, tick/undo score round-trip, staff affordance, conditional complete→residuals→reopen — state-restoring, passing 13s) |
| 0.1.8 | 2026-07-18 | **Correction — password back-dating reverted (audit find):** v0.1.7's item (3) assumed core prompts an expired-within-grace user to change their password; that prompt (`pwd_expires_alert.php`) is wired only into the legacy frames landing (`main_screen.php`) — the modern tabs shell shows **nothing**, making back-dating a silent grace-window countdown to login rejection. Temp passwords are stamped NOW again; the handover instruction is the change-password channel. Do not re-attempt back-dating unless the tabs shell gains the prompt |
