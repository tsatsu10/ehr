...
| `patients.chart.activity_feed` (spec name `mrd.activity_feed`) | `{ pid, offset?, limit?, visit_id? }` | `{ items: ActivityFeedItemDto[], has_more, next_offset }` — `visit_id` filters feed (Visit Board **View full history**) |
...
14. `patients.chart.activity_feed?visit_id=` returns only that visit’s events (M2-F12 deep link).
...
|---------|------|---------|
| 0.2.37 | 2026-07-09 | **Implementation audit closure** — §17 #6 `hide_dashboard_cards` mapping implemented (`resolveHiddenSections` + Clinical tab filter); §17 #1–#18 otherwise verified against code/tests (feed 25/90d/365 extend, visits 20/page, `visit_id` filter, D-MRD-13 `PatientChartTabResolver` + test); §8.7/§17 action names aligned to shipped `patients.chart.*` namespace |
