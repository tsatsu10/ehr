...
| [NEW_CLINIC_V1_ADMIN_CONFIGURATION_REDESIGN.md](./done/NEW_CLINIC_V1_ADMIN_CONFIGURATION_REDESIGN.md) | M6 + M15 Admin Hub | Draft v0.2.0 — Apple-theme nav redesign (ADM-1..7, see companion plan below) + per-clinic setting override transparency |
...
| [NEW_CLINIC_V1_CLINICAL_DOCUMENTATION_REDESIGN.md](./done/NEW_CLINIC_V1_CLINICAL_DOCUMENTATION_REDESIGN.md) | M4 + M17 Clinical Documentation Hub | **Built + always-on v0.1.4** (2026-07-19: post-visit corrections — closed visits open forms via bridge, manager unlock works after close) |
...
| [NEW_CLINIC_V1_CERTIFICATE_EYE_EXAM_REDESIGN.md](./new/NEW_CLINIC_V1_CERTIFICATE_EYE_EXAM_REDESIGN.md) | **Medical certificate + Eye exam** — numbered, auditable excuse-duty certificate with letterhead print + verify-by-number (replaces the thin fraud-prone stock note form), and a WHO-primary-care-level eye exam (R/L chips, 6/x acuity, refer bridge, optional spectacle Rx — replaces the ~16-table stock `eye_mag`); flags `enable_native_certificate` / `enable_native_eye_exam` | **Built + polished v0.4.0** (2026-07-19: verify-by-number, cert fee option, IOP toggle, spectacle print, refer→referral-composer bridge — no open items) |
...
| [NEW_CLINIC_BACKUP_SYSTEM_DESIGN.md](./new/NEW_CLINIC_BACKUP_SYSTEM_DESIGN.md) | In-app encrypted backup engine design: two-entity DB + incremental site-files backup, verify, cloud off-site without OAuth, recovery-key custody + export bundle (v0.9.0, **BACKUP-CAP/CAP2**: the encryption size cap is configurable and RAM-budget derived, its pre-check false-skip margin corrected to 20x (was 8x, itself a fix from an original 3x), proven on a real 1.7 GB dev DB with a decrypt round-trip; **M1**: full-stream verify + disk-full write checks so a truncated backup can't verify green; **M6/M6b/M7/M7b**: honest 0=never retention (files + run rows, now including stale `running` rows past the lock TTL), mirror never clobbers its last-known-good copy and no longer grows unbounded siblings on an unchanged source; **H1b/M4b/M4c**: scheduled retries back off after a failure, backup config always lives at facility 0 on read AND write; **BACKUP-DOCS/BACKUP-OFFSITE**: off-site claims corrected to reality — no automatic vendor-server replication exists, that is a named future task, not built) |
...
| [NEW_CLINIC_ADMIN_HUB_APPLE_REDESIGN_PLAN.md](./new/NEW_CLINIC_ADMIN_HUB_APPLE_REDESIGN_PLAN.md) | **ADM-1..8** — Admin Hub Apple-theme redesign: sticky grouped left-rail nav replaces the wrapped tab strip, mega-tab split into Queue & desks + Features, Setup promoted to its own destination, global settings search, unsaved-changes guard covers every navigation, one-accent visual pass, per-clinic setting override transparency (which also caught and fixed a real bug — every setting silently became "overridden" after the first save). **Executed — all 8 tasks shipped** (v1.7.0) |
...
| [NEW_CLINIC_REVIEW_VISIT_PRICING_PLAN.md](./new/NEW_CLINIC_REVIEW_VISIT_PRICING_PLAN.md) | **REV-1..5** — clinic-priced "review visit" for returning patients (same complaint, own chosen price + review window): `is_review` visit-type marker + clinic-priced `REVIEW_CONSULT` fee item (seeded inactive/zero), a leaf suggestion service, front-desk "Book as Review?" prompt with staff confirmation, zero cashier-path changes (rides the existing visit-type default-fee suggestion flow). Behind `enable_review_visits` (default OFF). **Delivered — built + HTTP-level live-smoked end to end 2026-07-19** (flag/type/fee toggled through the real admin ajax services, full queue walk to a posted cashier payment, all smoke state restored); PRD amendment queued (v1.0.0) |
