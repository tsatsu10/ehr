...
| [NEW_CLINIC_PATIENT_IMPORTER_PLAN.md](./new/NEW_CLINIC_PATIENT_IMPORTER_PLAN.md) | **MKT-MIG-1** — self-serve patient demographics CSV import in the Admin Hub: upload → match columns → dry-run report → import, duplicates skipped and reported, old clinic number carried over and made exact-match searchable; behind `enable_patient_import` (default OFF). **Delivered (V1) — browser-smoked 2026-07-17** (v0.2.0) |
