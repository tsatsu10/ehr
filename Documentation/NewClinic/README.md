...
| [NEW_CLINIC_ADMIN_HUB_APPLE_REDESIGN_PLAN.md](./new/NEW_CLINIC_ADMIN_HUB_APPLE_REDESIGN_PLAN.md) | **ADM-1..8** — Admin Hub Apple-theme redesign: sticky grouped left-rail nav replaces the wrapped tab strip, mega-tab split into Queue & desks + Features, Setup promoted to its own destination, global settings search, unsaved-changes guard covers every navigation, one-accent visual pass, per-clinic setting override transparency (which also caught and fixed a real bug — every setting silently became "overridden" after the first save). **ADM-1..7 shipped; ADM-8 (final verify+docs) in progress** (v1.6.0) |
