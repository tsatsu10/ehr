...
| [NEW_CLINIC_BACKUP_SYSTEM_DESIGN.md](./new/NEW_CLINIC_BACKUP_SYSTEM_DESIGN.md) | In-app encrypted backup engine design: two-entity DB + incremental site-files backup, verify, cloud off-site without OAuth, recovery-key custody + export bundle (v0.8.0, **BACKUP-CAP**: the encryption size cap is now configurable and RAM-budget derived, proven on a real 1.7 GB dev DB with a decrypt round-trip; **M1**: full-stream verify + disk-full write checks so a truncated backup can't verify green; **M6/M6b/M7**: honest 0=never retention (files + run rows), mirror never clobbers its last-known-good copy; **H1b/M4b**: scheduled retries back off after a failure, backup config always lives at facility 0 even on write) |
