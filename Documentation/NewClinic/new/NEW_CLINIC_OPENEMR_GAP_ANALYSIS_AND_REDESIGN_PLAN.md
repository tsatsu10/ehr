...
| W5 | Codes / superbill admin | Stock `superbill_custom_full.php` | Medium (fee schedule upkeep) | **RESOLVED (2026-07-13) — this row was stale AND the original scope was wrong.** Admin Hub's Fees tab already ships full fee-schedule CRUD (add/edit/archive/bulk-price/CSV import, GAP-C C2) — confirmed still current. `billing_code` on a fee line is free text, never validated against the core `codes` table (checked `CashierService`/billing-post path), so a cash clinic never actually needs the stock Codes screen for normal operation. The row's literal target, `superbill_custom_full.php`, IS what stock OpenEMR calls "Codes" (confirmed via `standard.json`'s own menu) but its scope centers on revenue codes / UB04 institutional-claims administration — insurance/EDI territory, a documented PRD non-goal for this cash-only product; building native CRUD for it would be out-of-scope work. **Real bug found and fixed:** the Fees tab's "Open OpenEMR Codes admin" escape-hatch link pointed at the wrong screen (`layout_service_codes.php`, a layout-attribute uploader unrelated to code administration) — corrected to `superbill_custom_full.php`. **Audit follow-up (2026-07-13):** the link swap alone was insufficient — `superbill_custom_full.php` gates on core `admin`/`superbill` (`AclMain::aclCheckCore('admin','superbill',...)`), and the New Clinic module's own `new_admin` GACL group only ever held `new_clinic`-section ACOs (confirmed against `library/classes/Installer.class.php`'s core seed: only `Administrators`/`Back Office`/`Emergency Login` get `admin.superbill`). The corrected link would have 403'd for every properly-provisioned New Clinic Admin user — neither the old nor the new target was reachable. Fixed by bridging `new_admin` → core `admin`/`superbill` in `acl_setup.php`'s existing `coreGrants` pattern (same mechanism already used for cashier→`acct.rep` and reception→`patients.pat_rep`), `AclVersion` bumped 0.2.8→0.2.9, verified live against the dev DB (`gacl_aco_map`/`gacl_aro_groups_map` rows confirmed present for ACL id 39 / group 28). Nothing else to build. |
...
| Version | Date | Change |
|---|---|---|
| v0.1.36 | 2026-07-18 | **Doc-sync fix, not a code change — the top banner and this table had drifted behind §5 itself.** Independently re-verified A1 (Office Notes) against current code before recording anything (island, ajax actions, ACL, companion pin table all present and working, per the method in CLAUDE.md §12 — trust the code, not a prior doc's wording): confirmed built, and confirmed it is now a **permanent, flag-free surface** — `enable_office_notes` was retired and the legacy `office_comments_full.php` redirect removed as part of the 2026-07-18 PRD §5.6 flag-retirement amendment. §5's own A1 entry (line 178) and the §8 conventions summary (line 916) already stated this correctly — only the top-of-doc version banner and this history table had never been bumped to match, so a reader skimming just those two spots would have seen stale "flag-gated" framing. No code touched; this row and the banner line are the only changes. Separately: a stale worktree (`.claude/worktrees/gap-a-office-notes`) was found holding a pre-build v0.1.1 draft of this file, 34 versions behind — its own commits (the original A1 build) were already merged to `main` long ago; the worktree itself was never used to make this edit. |
| v0.1.35 | 2026-07-13 | **Tech-debt audit of v0.1.33/34 (`e5ba6f83`/`d67a6d6e`) — one real gap found and fixed, everything else held up.** (1) W5's link fix alone was insufficient: `superbill_custom_full.php` gates on core `admin`/`superbill`, which the New Clinic `new_admin` GACL group never held (it only carries `new_clinic`-section ACOs) — confirmed against `Installer.class.php`'s core seed that only `Administrators`/`Back Office`/`Emergency Login` get that ACO, so the "fixed" link would have 403'd for every real New Clinic Admin user. Fixed by bridging `new_admin` to `admin`/`superbill` via `acl_setup.php`'s existing `coreGrants` pattern, `AclVersion` 0.2.8→0.2.9, verified live at the `gacl_*` table level (not just the install script's echo output). (2) W7's `abook_type` exclusion re-checked — citation (`addrbook_edit.php` ~L419-422) confirmed accurate; the other three allow-listed lists checked for the same option_value-branching risk. (3) W8 and B2b's claims re-verified fresh (code inspection for W8's `saveDrug`/CSV-import wiring; live WHO/CDC web search for B2b's licensing claim) — both held up unchanged. (4) Confirmed no concurrent-session files were touched. |
