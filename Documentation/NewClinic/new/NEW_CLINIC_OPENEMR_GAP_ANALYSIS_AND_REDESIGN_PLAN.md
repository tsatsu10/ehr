...
- **UI:** single-column feed of `WidgetCard` notes with author/date, active/archived `SegmentedControl` (Active/Archived/All), inline compose + inline edit, **pin toggle** (pinned notes float to the top with a Pinned badge + accent), `RowActionsMenu` (Edit / Archive-Restore / Delete), delete behind `ConfirmModal`.
- **Backend:** `ajax.php` actions `onotes.list|save|archive|pin|delete` via `OfficeNotesService`. Writes delegate to core `OpenEMR\Services\ONoteService` (core `onotes` untouched); **pin state lives in the module-owned companion table `new_office_note_meta`** (`onote_id` PK) — the list read LEFT JOINs it so pinned notes sort first globally, not just within a page.
