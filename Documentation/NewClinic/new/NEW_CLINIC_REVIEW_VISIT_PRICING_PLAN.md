...
**Version:** v1.0.1 · **Date:** 2026-07-19 · **Status:** Delivered — REV-1..5 shipped, built, and live-smoked end to end; REV-6 final-review fixes landed (CPT4 code seed so the price is actually settable, inactive fee seed, same-facility suggestion scoping) — see History for details (HTTP-level smoke; browser click-through still recommended at next convenient window)
...
| v1.0.1 | 2026-07-19 | **REV-6 — final-review fixes.** (A1) Seeded a CPT4 `codes` row for `REVIEW_CONSULT` in install.sql (idempotent `#IfNotRow codes code REVIEW_CONSULT`, INSERT..SELECT ct_id FROM code_types) — closes the exact gap the REV-5 smoke note flagged as a "pre-existing environment gap": a stock install ships CPT4 empty, so `FeeScheduleAdminService::assertBillingCodeExists` blocked every attempt to price the review fee and it would have stayed stuck at 0.00 (free) forever. Applied to the dev DB (row id 268, active=1). (A2) Changed the `REVIEW_CONSULT` fee seed to `is_active = 0` (was 1) — an active 0.00 fee was appearing in every cashier fee picker regardless of the `enable_review_visits` flag, breaking "flag OFF = 100% legacy" (PRD §5.6); `FeeScheduleAdminService::save()` sets `is_active = 1` automatically the moment the clinic prices it, so no extra admin step is needed beyond what REV-5's smoke already documented. Applied to the dev DB (row id 11). (A3) `ReviewVisitSuggestionService::suggestFor()`'s anchor query was cross-facility (`WHERE fe.pid = ?` only); added `AND fe.facility_id = ?`, still seeking the `(pid, date)` index on the pid equality. Proved live (CLI, flag toggled via `ClinicConfigService::set`/`invalidate`, reverted after): patient 557 (encounter today at facility 3) → `suggestFor(557, 3)` populated, `suggestFor(557, 14)` and `suggestFor(557, 999)` both `null`. (B1) Did **not** thread `PatientContextService`'s existing `last_visit` `MAX(date)` into `suggestFor()` to save a query — that lookup is cross-facility and passing it through would have silently reintroduced A3's bug; documented with a comment at both call sites instead. (B2) `useStartVisit.ts`'s `applyReviewSuggestion()` now keys off the filtered `reviewSuggestion` (type confirmed present + not already selected) instead of the raw, unfiltered `rawSuggestion`. (B3) Added two more `daysSince()` garbage-input cases proven live by the reviewer: `'99'` and `'2026-13-45'` both → null. Gates: `ReviewVisitSuggestionServiceTest` green (5 tests, 15 assertions); `composer verify:new-clinic` PASS (302 files, 0 cycles); frontend Vitest green; build + `ModuleAssetVersion` bumped. **Two items accepted for later, not fixed here:** (i) a facility-level fee with the same label "Review" would silently shadow the global `is_review`-marked row — `FeeScheduleAdminService::listForAdmin` doesn't surface which row is the review-marked one, so there is no admin-facing warning/badge today if a facility overrides it; (ii) the seed-guard granularity note — `#IfNotRow codes code REVIEW_CONSULT` checks only the `code` column (not code_type), which is correct for this module-owned code but would need tightening if `REVIEW_CONSULT` were ever reused across code types. Full transcript: `.superpowers/sdd/rev-6-report.md`. |
| v1.0.0 | 2026-07-19 | **REV-5 delivered.** Built the frontend (`ModuleAssetVersion` bumped `-review1`) and re-ran `composer verify:new-clinic` (PASS, 302 files, 0 constructor cycles, 366 ajax actions crosschecked). The shared Playwright MCP browser was in use by another session (`ms-playwright-mcp` Chrome processes present), so the chain was proven with an HTTP-level throwaway smoke script (scratchpad, not committed) driving the real ajax actions as an admin/front-desk/doctor/cashier would, not a browser click-through — noted honestly, not overstated as a browser smoke. Confirmed end to end: (1) flag ON + window 14 saved via `admin.config.save`; (2) the seeded-inactive Review visit type activated via `admin.visit_type.save` (confirms the REV-1 `is_active=0` seed decision — flag OFF alone is not enough, the clinic must also deliberately activate the type in Visit types, exactly as designed); (3) the `REVIEW_CONSULT` fee priced to 10.00 via `admin.fee.save` — this surfaced a real, feature-unrelated dev-DB gap (the CPT4 `codes` table was essentially empty, so the fee-save billing-code validation that already existed for every fee item, including the pre-existing `OPD_CONSULT` seed, blocked the save; worked around for the smoke only by inserting a throwaway CPT4 `codes` row, deleted immediately after — this is a pre-existing environment gap, not a review-visit bug, but worth a follow-up seed fix); (4) `patients.preview` (front-desk context) returned a populated `review_suggestion` for a patient with a same-day encounter and `null` for one with none; (5) `visit.types` returned the Review row with `is_review: true`; (6) a real visit was started typed Review (manager override used to pass an unrelated pre-existing revisit-completeness gate on the test patient), walked skip-triage → doctor take → doctor complete → `ready_for_payment`, and `cashier.select`'s `suggested_fees` showed exactly one line — `REVIEW_CONSULT` at price 10.00 — which was then posted and paid (e-sign/completion admin overrides used, same pattern as other pilot/smoke flows) for a real receipt; (7) flag flipped back OFF and `patients.preview` for the same patient (still with a same-day encounter) returned `review_suggestion: null` again, proving the flag — not just encounter recency — gates the prompt. All smoke state fully restored: `enable_review_visits` back to `0`, Review visit type back to `is_active=0`, `REVIEW_CONSULT` price back to `0.00`, the temporary CPT4 code row deleted, and every DB row created by the smoke visit (`new_visit`, `form_encounter`, `billing`, `payments`, `ar_activity`, `ar_session`, `new_visit_state_log`) deleted — verified back to the exact pre-smoke baseline. Full transcript in `.superpowers/sdd/rev-5-report.md`. **PRD amendment queued** — not yet written into the PRD proper; this plan + the README row are the record until then. |
