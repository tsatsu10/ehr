...
| 2026-07-19 | Engineering | **Full Playwright e2e suite GREEN (140/140, one continuous 27.7-min run)** after a five-round drift-repair campaign (22 → 0 failures, no product code changed except the DR-08 unlock gate + session-lock fixes recorded above). Root causes fixed in the suite: triage desk auto-saves vitals (specs now wait for the saved STATE, not the response); persistent Patient-ready toast from leftover fixture visits intercepted toolbar clicks suite-wide (shared prep now janitors fixture visits per spec boundary; refresh clicks toast-tolerant); `enable_hard_provider_assignment` persisting turns Send-to-doctor into a chooser dialog (every send confirms it conditionally); Playwright role-name substring matching, Radix `data-state`, HTML5-dnd synthetic dispatch, and a long tail of redesign-selector drift (SegmentedControl tabs, chat header actions, de-bootstrapped drawers, admin sidebar links, removed US-gateway vault). |
