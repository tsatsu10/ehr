...
/**
 * Ensure a waiting visit exists for V1.1-LAB-ORD E2E; emit JSON fixture.
 *
 * Usage:
 *   php interface/modules/custom_modules/oe-module-new-clinic/scripts/v11-lab-ord-smoke-fixture.php
 */
...
$panelOrder = new LabPanelOrderService();
$catalogEnabled = $panelOrder->isFeatureEnabled($facilityId);
...
if (!is_array($existing) || empty($existing['visit_id'])) {
    ...
    $visit = (new VisitQueueService())->startVisit($pid, $visitTypeId, $actorId, $facilityId, 'E2E lab panel order smoke');
    $existing = [
...
echo json_encode([
    ...
    'catalog_has_tests' => $testCount > 0,
    'catalog_test_count' => $testCount,
