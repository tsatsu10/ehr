...
/**
 * Ensure a waiting visit exists for V1.2-PHARM-RX E2E; emit JSON fixture.
 *
 * Usage:
 *   php interface/modules/custom_modules/oe-module-new-clinic/scripts/v12-pharm-rx-smoke-fixture.php
 */
...
$formularyRx = new PharmFormularyRxService();
$catalogEnabled = $formularyRx->isFeatureEnabled($facilityId);
...
if (!is_array($existing) || empty($existing['visit_id'])) {
    ...
    $created = $patientService->insert([
        ...
        'sex' => 'Female',
        'pubpid' => $pubpid,
        'phone_cell' => '0247444' . substr($suffix, -4),
    ]);
    if (!$created->isValid()) {
    ...
    $visit = (new VisitQueueService())->startVisit($pid, $visitTypeId, $actorId, $facilityId, 'E2E formulary quick Rx smoke');
    $existing = [
...
echo json_encode([
    ...
    'catalog_has_drugs' => $drugCount > 0,
    'catalog_drug_count' => $drugCount,
