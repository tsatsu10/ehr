...
class ClinicRolesService
{
     ...
     *
     * MUST mirror `interface/modules/custom_modules/oe-module-new-clinic/acl/acl_setup.php`'s
     * `$acos` array exactly -- this is a read-only *display* copy for the Admin Hub ACL
     * Inventory / Sensitive Permissions summary, not the source of truth (acl_setup.php's
     * AclExtended::addObjectAcl() calls are). It cannot safely `require` acl_setup.php at
     * runtime (that script re-runs live GACL install side effects and requires
     * $aclSetupFlag === true), so this list must be updated by hand whenever acl_setup.php's
     * $acos/$deskMap/$extraGrants/$roleAddonGrants change. A stale copy doesn't break access
     * control (grants happen at install time from the real file) -- it just makes this
     * inventory screen silently under-report what a role can already do. Found drifted
     * 2026-07-11 (missing ~44 ACOs and most lead/doctor extra grants); resynced here.
     */
    private const ACOS = [
    ...
    public static function buildGrantMatrix(): array
    {
        ...
        $extraGrants = [
            ...
            'new_lab_lead' => ['new_lab_lead', 'new_lab', 'new_lab_ops', 'new_lab_ops_enter', 'new_lab_ops_release', 'new_lab_order_intake'],
            'new_pharmacy_lead' => ['new_pharmacy_lead', 'new_pharmacy', 'new_pharm_ops', 'new_pharm_ops_dispense', 'new_pharm_ops_receive', 'new_pharm_ops_destroy', 'new_pharmacy_undispensed_override', 'new_pharmacy_external_rx_override', 'new_pharmacy_walkin_dispense', 'new_pharmacy_refer_to_opd', 'new_reports_pharmacy'],
            'new_cashier_lead' => ['new_cashier_lead', 'new_cashier', 'new_billing_skip_completion', 'new_discount', 'new_visit_mark_outstanding', 'new_close_without_charge', 'new_receipt_reprint', 'new_esign_skip_complete', 'new_chart_depth', 'new_chart_depth_finance', 'new_chart_depth_referral', 'new_bill_ops', 'new_bill_ops_correct', 'new_bill_ops_payment', 'new_bill_ops_close', 'new_bill_ops_outstanding', 'new_reports_financial'],
