...
export function RecentlyViewed({ patients, onSelect, onClear }: RecentlyViewedProps) {
  ...
  return (
    <div className="nc-recently-viewed flex items-center gap-2 flex-wrap px-3 py-2 border-b border-[var(--oe-nc-border)] bg-[var(--oe-nc-bg-tint)]">
      ...
      <ul className="flex flex-wrap gap-1.5 list-none p-0 m-0 flex-1 min-w-0">
        {patients.map((patient) => (
          <li key={patient.pid}>
            <button
              type="button"
              className="nc-recently-viewed-pill inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white border border-[var(--oe-nc-border)] text-xs font-medium text-[var(--oe-nc-text)] hover:border-[var(--oe-nc-primary)] hover:text-[var(--oe-nc-primary)] transition-colors max-w-[14rem]"
