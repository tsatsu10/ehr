...
export const QUEUE_FIELD_SECTIONS: AdminFieldSection[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      {
        key: 'enable_otc_manual_discount',
        type: 'bool',
        label: 'Allow a discount on over-the-counter sales',
        hint: 'When on, the pharmacist can lower the amount on an OTC sale below the listed price (a discount); the amount can never go above the listed price. Default OFF = the amount is always quantity × unit price.',
        indent: 1,
      },
      {
      ...
      {
        key: 'enable_native_rx_history',
        type: 'bool',
        label: 'Native Prescription History',
        hint: 'Replaces the stock, patient-wide Rx list with a native paginated history page (view + print only, no bulk actions). OFF = stock list. Independent of Pharmacy Operations hub.',
        indent: 1,
      },
      {
  ...
  },
  {
    ...
    fields: [
      ...
      {
        key: 'enable_native_immunization_editor',
        type: 'bool',
        label: 'Native immunization editor (D-IMM-1)',
        hint: 'When on, Add/Edit on the chart Immunizations section opens a native drawer with a Ghana EPI vaccine list (BCG, OPV, Pentavalent, PCV, Rotavirus, Measles-Rubella, Yellow Fever, etc.) instead of the stock immunizations screen. Records the vaccine, date given, here/elsewhere, lot, and a note to the same immunization record. patients/med write ACL enforced. Default OFF.',
        indent: 1,
      },
      {
      ...
      {
        key: 'certificate_auto_bill',
        type: 'bool',
        label: 'Certificate attracts a fee',
        hint: 'When on, issuing a medical certificate automatically adds the certificate fee to the visit bill, so the cashier collects it at checkout. Set the price on the Fee schedule under the code MED_CERT (the certificate saves either way — if that price row is missing or inactive, no charge is added and the clinician sees a note). Re-issuing or amending a certificate on the same visit never charges twice. Default OFF.',
        indent: 2,
      },
      {
      ...
      {
        key: 'eye_exam_show_iop',
        type: 'bool',
        label: 'Eye exam shows eye pressure (IOP)',
        hint: 'Keep this on if the clinic owns a tonometer. Turn it off to hide the eye-pressure section from the eye exam so nobody records pressures the clinic cannot actually measure. Default ON.',
        indent: 2,
      },
    ],
  },
  ...
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        hint: 'Delete backup files (and their run history rows) older than this many days. 0 = never delete — keep every backup forever. Default 30.',
        min: 0,
        max: 365,
        indent: 1,
      },
      ...
      },
      {
        ...
        hint: 'How often a scheduled backup should run (0 = off). A backup is flagged "due" once this many days pass since the last successful one. Being logged in with a tab open is NOT enough on its own — schedule scripts/run-jobs.php (or scripts/backup-scheduled.php) via Windows Task Scheduler or Linux cron to actually run it. See the "Schedule automatic backups" runbook for the exact command.',
        min: 0,
        max: 365,
        indent: 1,
      },
      ...
      {
        key: 'backup_max_encrypt_mb',
        type: 'int',
        label: 'Backup max size to encrypt (MB)',
        hint: 'Largest COMPRESSED database backup this machine will encrypt in memory. Default 250 MB assumes a modest 2 GB RAM server. If a scheduled backup keeps failing as "too large", and this machine has more free RAM, raise this number — each 250 MB of headroom here needs roughly 1 GB of free RAM during the backup. If your database is genuinely bigger than this machine can handle in memory, use the VPS replica or the stock backup screen instead.',
        min: 50,
        max: 2000,
        indent: 1,
      },
  ...
  },
  {
    ...
    description: 'Form bundles and screening/specialty lenses. The hub and the native consult engine are always on (retired 2026-07-18).',
    fields: [
      {
        key: 'consult_note_formdir',
        type: 'string',
        label: 'Legacy consult note form directory',
        hint: 'Registry formdir of pre-flip consult notes (default soap) — those notes keep opening in the native consultation form.',
        indent: 1,
      },
      {
      ...
      },
      {
        ...
        hint: 'Adds the Screening lens to the always-on Clinical Documentation Hub.',
        indent: 1,
      },
      {
        ...
        hint: 'Requires the clinical_doc_specialty_pack JSON below.',
        indent: 1,
      },
      {
