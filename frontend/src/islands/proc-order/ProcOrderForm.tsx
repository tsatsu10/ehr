...
export function ProcOrderForm({
  ...
  if (loading) {
    return <p className="nc-proc-order__status">{t('Loading the order form…')}</p>;
  }
  if (loadError) {
    return (
  ...
  const specimenLabel = t('Specimen type');

  return (
    <div className="nc-proc-order">
      <header className="nc-proc-order__head">
      ...
      <div className="nc-proc-order__grid">
        <div className="nc-proc-order__field">
          <Label htmlFor="nc-proc-order-lab">{t('Lab')}</Label>
          <NativeSelect
        ...
        <div className="nc-proc-order__field">
          <Label htmlFor="nc-proc-order-priority">{priorityLabel}</Label>
          <NativeSelect
            id="nc-proc-order-priority"
            value={priority}
      ...
      <fieldset className="nc-proc-order__tests">
        <legend className="nc-proc-order__legend">{t('Tests')}</legend>
        {tests.length === 0 ? (
          ...
          <ul className="nc-proc-order__test-list">
            {tests.map((test) => (
              <li key={test.procedure_type_id} className="nc-proc-order__test">
                <Checkbox
        ...
        {estimatedTotal != null && (
          <p className="nc-proc-order__total">
            {t('Estimated: {total}', { total: formatMoney(data.currency_symbol, estimatedTotal) })}
          </p>
        )}
      </fieldset>
      ...
      <div className="nc-proc-order__grid">
        <div className="nc-proc-order__field">
          <Label htmlFor="nc-proc-order-specimen">{specimenLabel}</Label>
          <NativeSelect
      ...
      <div className="nc-proc-order__field">
        <Label htmlFor="nc-proc-order-hx">{t('Clinical history')}</Label>
        <Textarea
          id="nc-proc-order-hx"
          rows={2}
          value={clinicalHx}
      ...
      <div className="nc-proc-order__actions">
        ...
        <Button type="button" disabled={saving || selected.size === 0} onClick={() => void submit()}>
